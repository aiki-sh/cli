...
| Enable-marker fallback to the source repo | The workspace checks out the repo's committed `.aiki/`, so the init-state gate (`repos::init_state`) anchors its root on the workspace path. But the per-user enable marker is keyed to the *source* repo the user ran `aiki init` in, not the throwaway workspace. Without a fallback, every `aiki` command run from inside an isolated workspace would be a false-negative `Dormant` ("not enabled for this account") — and that message tells the agent to stop using aiki, the exact wrong outcome inside an aiki-created workspace. `init_state` resolves the source repo via `find_repo_root_from_workspace` (reads the workspace's `.jj/repo`) and checks that marker before declaring `Dormant`. Safe no-op for non-workspace repos, where `.jj/repo` is a directory and the resolver returns `None` |
